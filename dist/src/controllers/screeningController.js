...
import { getRiskCategory, getPriorityDomains, getMainIndicationByPriorityDomains, getIndicationSummary, getGeneralRecommendationText, getResultDescription, getScreeningQuestionsByAgeMonth, } from "../helper/screeningHelper.js";
import { generateActivitiesForChild } from "./activityController.js";
import { logUserActivity } from "../utils/logger.js";
export const getScreeningQuestionsByChild = async (req, res) => {
    try {
        if (!req.user) {
...
export const startScreening = async (req, res) => {
    try {
        ...
        if (questions.length === 0) {
            return errorResponse(res, "Pertanyaan screening untuk usia anak ini belum tersedia", 400);
        }
        ...
        return successResponse(res, "Sesi screening berhasil dimulai", {
            session,
            child: {
                id: child.id,
                name: child.name,
...
export const submitScreening = async (req, res) => {
    try {
        ...
        const questionIds = answers.map((item) => String(item.questionId));
        const uniqueQuestionIds = [...new Set(questionIds)];
        const hasInvalidAgeQuestion = uniqueQuestionIds.some((questionId) => !allowedQuestionIds.includes(questionId));
        if (hasInvalidAgeQuestion) {
            return errorResponse(res, "Terdapat pertanyaan yang tidak sesuai dengan usia anak", 400);
        }
        if (uniqueQuestionIds.length !== answers.length) {
            return errorResponse(res, "Terdapat pertanyaan yang dijawab lebih dari satu kali", 400);
        }
        if (answers.length !== allowedQuestions.length) {
            return errorResponse(res, `Jawaban belum lengkap. Total pertanyaan adalah ${allowedQuestions.length}, tetapi jawaban yang dikirim ${answers.length}.`, 400);
        }
        ...
        if (questions.length !== uniqueQuestionIds.length) {
            return errorResponse(res, "Ada pertanyaan yang tidak valid atau tidak aktif", 400);
        }
        const domainScores = {
        ...
        const domainPercentages = {
            COMMUNICATION_SPEECH: domainMaxScores.COMMUNICATION_SPEECH
                ? Number(((domainScores.COMMUNICATION_SPEECH /
                    domainMaxScores.COMMUNICATION_SPEECH) *
                    100).toFixed(2))
                : 0,
            PHYSICAL_MOTOR: domainMaxScores.PHYSICAL_MOTOR
                ? Number(((domainScores.PHYSICAL_MOTOR /
                    domainMaxScores.PHYSICAL_MOTOR) *
                    100).toFixed(2))
                : 0,
            COGNITIVE_PROBLEM_SOLVING: domainMaxScores.COGNITIVE_PROBLEM_SOLVING
                ? Number(((domainScores.COGNITIVE_PROBLEM_SOLVING /
                    domainMaxScores.COGNITIVE_PROBLEM_SOLVING) *
                    100).toFixed(2))
                : 0,
            SOCIAL_EMOTIONAL: domainMaxScores.SOCIAL_EMOTIONAL
                ? Number(((domainScores.SOCIAL_EMOTIONAL /
                    domainMaxScores.SOCIAL_EMOTIONAL) *
                    100).toFixed(2))
                : 0,
        };
        const finalScore = Number((domainPercentages.COMMUNICATION_SPEECH *
            ...
            domainPercentages.SOCIAL_EMOTIONAL * DOMAIN_WEIGHTS.SOCIAL_EMOTIONAL).toFixed(2));
        ...
        const mainIndication = getMainIndicationByPriorityDomains(priorityDomains, finalScore);
        const indicationSummary = getIndicationSummary(category, priorityDomains);
        const resultDescription = getResultDescription(finalScore, category, priorityDomains, domainPercentages);
        const recommendationText = getGeneralRecommendationText(category, priorityDomains);
        const recomendationActivity = await generateActivitiesForChild(session.child.id, session.id, age.ageMonth, priorityDomains, mainIndication);
        const result = await prisma.$transaction(async (tx) => {
            ...
            const updatedSession = await tx.screeningSession.update({
                ...
                data: {
                    ...
                    cognitiveProblemSolvingScore: domainScores.COGNITIVE_PROBLEM_SOLVING,
        ...
        return successResponse(res, "Screening berhasil diselesaikan", {
            ...
            calculation: {
                ...
                priorityDomainLabels: priorityDomains.map((domain) => DOMAIN_LABELS[domain]),
                weights: DOMAIN_WEIGHTS,
                domainScores,
                domainMaxScores,
                domainPercentages,
                disclaimer: "Hasil screening ini bukan diagnosis final dan tidak menggantikan pemeriksaan profesional.",
            },
...
export const getScreeningResult = async (req, res) => {
    try {
        if (!req.user) {
            return errorResponse(res, "User tidak terautentikasi", 401);
        }
...
export const getScreeningHistoryByChild = async (req, res) => {
    try {
        if (!req.user) {
            return errorResponse(res, "User tidak terautentikasi", 401);
        }
        ...
        return successResponse(res, "Riwayat screening berhasil diambil", histories);
    }
    catch (error) {
