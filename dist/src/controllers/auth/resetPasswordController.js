...
export const resetPassword = async (req, res) => {
    try {
        ...
        if (!email || !otp || !newPassword || !confirmPassword) {
            return errorResponse(res, "Email, OTP, password baru, dan konfirmasi password wajib diisi", 400);
        }
        if (!isValidPassword(String(newPassword))) {
            return errorResponse(res, "Password baru minimal 8 karakter dan harus memiliki kombinasi huruf serta angka", 400);
        }
        ...
        const hashedPassword = await bcrypt.hash(String(newPassword), Number(process.env.BCRYPT_SALT_ROUNDS) || 10);
