...
export const register = async (req, res) => {
    try {
        ...
        if (!fullName || !email || !password || !confirmPassword) {
            return errorResponse(res, "Nama lengkap, email, password, dan konfirmasi password wajib diisi", 400);
        }
        ...
        if (!isValidPassword(String(password))) {
            return errorResponse(res, "Password minimal 8 karakter dan harus memiliki kombinasi huruf serta angka", 400);
        }
        ...
        const hashedPassword = await bcrypt.hash(String(password), Number(process.env.BCRYPT_SALT_ROUNDS));
        ...
        try {
            ...
            return errorResponse(res, "Gagal mengirim OTP ke email. Silakan coba lagi nanti.", 500);
        }
        logUserActivity({
            userId: user.id,
            action: "REGISTER",
        ...
        return successResponse(res, "Register berhasil. Silakan verifikasi OTP.", {
            user,
